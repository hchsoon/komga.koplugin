...
- 新增: 设置 → 缓存与维护 → "从 Komga 同步阅读历史"(手动触发)。全库按 readProgress.lastModified
  ...
  有 spec 回归防线
- 历史同步二期: 书架上没出现过的系列也能建快捷方式——卷行按需落地(volume 表只含进过的系列,
  用 BookDto 自带字段直接 upsert 单行; 系列行也缺失时拉单条 SeriesDto 建行, upsertSeries 走
  isUpdate=true 避免触发"停用全部旧系列"), 并顺带补建书架首页的系列快捷方式(addBookShortcut 幂等)
- 修复: 点击指向已删除分卷的历史条目闪退(两层): Reader 内点历史走 aux provider 无 callback
  分支, 回退 ui["komga"]:openFile 而 ReaderUI 从未赋值该字段——补 callback 显式路由到插件打开
  流程; 文件管理器路径对失效快捷方式移除对应历史条目并提示, 不再走 showReader(不存在文件)
- 历史同步自动档: 设置新增"书架刷新后自动同步历史"开关(默认关), 开启后每次书架手动刷新成功
  即自动跑一次同步(fetchAllPages 支持 max_pages 截断, 倒序取前缀场景单页拿齐最近 max_fetch 条,
  不再全库翻页)
- 修复: 历史同步建出的系列首次进入只显示读过的分卷(卷行部分落地被"#行数>0 即已同步"误判)——
  首次进入系列时行数不足系列 booksCount 先补拉全量卷清单, 再落全部分卷快捷方式
