...
function ChapterListing:generateItemTableFromChapters(chapters)
    ...
    for _, chapter in ipairs(chapters) do
        ...
        table.insert(item_table, {
            ...
            text = chapter.title or tostring(chapter.number),
            ...
            local autoturn_spin = SpinWidget:new{
                ...
                callback = function(autoturn_spin)
                    ...
                    local status, err = pcall(function()
                        ...
                        self:ChapterDownManager(tonumber(number), 'next', autoturn_spin.value)
                    end)
...
function ChapterListing:ChapterDownManager(begin_number, call_event, down_chapters_count, dismiss_callback,
    cancel_callback)

    if not H.is_num(begin_number) then
    ...
    local begin_chapter = Backend:getVolumeInfoCache(book_cache_id, begin_number)
    
    begin_chapter.call_event = call_event

    if not H.is_tbl(begin_chapter) or begin_chapter.number == nil then
    ...
    if H.is_tbl(chapter_down_tasks) and chapter_down_tasks[1] ~= nil and chapter_down_tasks[1].book_cache_id ~= nil then
        ...
        local dialog = require("Komga/DownloadUnreadChaptersJobDialog"):new{
            show_parent = self,
            job = job,
...
function ChapterListing:showEpubToc(chapter)
    -- 获取 epub 内部目录
    ...
    -- 创建 epub 目录菜单
    local epub_toc_menu = Menu:new{
        title = string.format("%s - 内部目录", chapter.title),
        subtitle = "epub 章节导航",
        is_popout = false,
        ...
        items_per_page = G_reader_settings:readSetting("toc_items_per_page") or self.toc_items_per_page_default,
        items_font_size = G_reader_settings:readSetting("toc_items_font_size") or Menu.getItemFontSize(G_reader_settings:readSetting("toc_items_per_page") or self.toc_items_per_page_default)
    }
    ...
    epub_toc_menu.paths = {{
        callback = function()
            UIManager:close(epub_toc_menu)
        end
    }}

    -- 显示目录菜单
    UIManager:show(epub_toc_menu)
end

function ChapterListing:syncProgressShow(chapter)
    Backend:closeDbManager()
    MessageBox:loading("同步中 ", function()
        if H.is_tbl(chapter) and H.is_num(chapter.number) then
